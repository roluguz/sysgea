...
?>
    <div class="box box-default">
      <div class="box-header with-border">
        <h1>Gesti&oacute;n Especializaciones</h1>
      </div>
      <div class="row">
        ...
        <div class="col-md-4">
          <?php
          $cod    = "";
          $btnnew  = "<a class='btn btn-success btn-block btn-flat' title='Adicionar una Especializaci&oacute;n' ";
          $btnnew .= "type='button' datacode='new' href='?page=frmgralspecial&op=4'";
          $btnnew .= ">Agregar Especializaci&oacute;n</a>";
          echo $btnnew;
          ?>
        </div>
          ...
          <div class="col-md-10">
            <div class="box-body">
              <table id="example1" class="display table table-bordered table-hover">
                <thead>
                  <tr>
                    <th>Nombre Especializaci&oacute;n</th>
                    <th>Acciones</th>
                  </tr>
                </thead>
                <tbody class="contenidobusqueda">
                  ...
                  while ($row = mysqli_fetch_assoc($obSpecial)) {
                    ...
                    $openlace = ' type="button" href="?page=frmgralSpecial';
                    $inispan = '<span ';
                    $finspan = '</span>';
                    if ($row['estado'] == 1) { // esta activo   
                      ...
                      $opEdit .= $openlace . '&op=3&id=' . $row['idSpecial'] . '">';
                      $opEdit .= $figedit . $finref;
                      if ($row['nro'] == 0) {
                        $opErase  = $iniref . $opErase;
                        $opErase .= $openlace . '&op=2&id=' . $row['idSpecial'] . '">';
                      ...
                      $opActive .= $openlace . '&op=2&id=' . $row['idSpecial'] . '">';
                      $opActive .= $figact . $finref;
                    }
                    $btn = $opEdit . ' ' . $opErase . ' ' . $opActive
                  ?>
                    <tr>
                      <td><?php echo trim($row['nomSpecial']); ?></td>
...
<?php
  ...
  case 2: // Borrar
    ...
    $nnn = 'erase';
    $strAction = "Borrar";
    $tit   = "Borrar Especializaciones Asesores";
  ...
  case 3:  // Editar
    $nnn = 'edita';
    $strAction = "Editar";
    $tit   = "Edici&oacute;n Especializaciones Asesores";
  ...
  case 4: // Nuevo registro, revisar     
    $nnn = 'new';
    $strAction = "Nuevo";
    $tit   = "Adici&oacute;n Especializaciones Asesores";
...
function generaHTML($xx, $tit, $strAction, $strb1, $strb2, $idbusca, $nnn, $onlyread)
{
  ...
  if ($xx != 2) { //cuando no es un nuevo registro    
    ...
    if ($n == 0) { // no se encuentra el registro
      $p = "tipodoc";
      $enlace = "?page=frmgralSpecial&op=1";
      include('../plantillas/paso.php');
    } else $rs = mysqli_fetch_assoc($obfn);    
  }
  $headform = '<form autocomplete="off" action="../Controllers/regSpecialController.php" method="post">';
  $strHTML = "";
  $strHTML =
    '<div class="box box-default">
      <div class="box-header with-border">
              ...
              <div class="row">
                ...
                <div class="col-md-8">
                  <div class="form-group has-feedback">
                    <label>Descripci&oacute;n especializaci&oacute;n</label>
                    <input type="text" name="nomSpecial" id="nomSpecial" class="form-control" 
                    placeholder="Descripcion" title = "Descripci&oacute;n especializaci&oacute;n"
                    value="' . (($xx == 1) ? $rs['nomSpecial'] : '') . '" required
                    ' . $onlyread . '>
