...
?>
    <div class="box box-default">
      <div class="box-header with-border">
        <h1>Gesti&oacute;n tipo documentos</h1>
      </div>
      <div class="row">
        ...
        <div class="col-md-4">
          <?php
          $cod    = "";
          $btnnew  = "<a class='btn btn-success btn-block btn-flat' title='Adicionar un tipo documento' ";
              ...
              <table id="example2" class="display table table-bordered table-hover">
                ...
                <tbody class="contenidobusqueda">
                  ...
                  while ($row = mysqli_fetch_assoc($obSpecial)) {
                    ...
                    $openlace = ' type="button" href="?page=frmgraltipoDocumento';
                    ...
                    if ($row['estado'] == 1) { // esta activo   
                      ...
                      $opEdit .= $openlace . '&op=3&id=' . $row['idtipoDocumento'] . '">';
                      $opEdit .= $figedit . $finref;
                      if ($row['nro'] == 0) {
                        $opErase  = $iniref . $opErase;
                        $opErase .= $openlace . '&op=2&id=' . $row['idtipoDocumento'] . '">';
                      ...
                      $opActive .= $openlace . '&op=2&id=' . $row['idtipoDocumento'] . '">';
                      $opActive .= $figact . $finref;
                    }
                    $btn = $opEdit . ' ' . $opErase . ' ' . $opActive
                  ?>
                    <tr>
                      <td><?php echo trim($row['Descripcion']); ?></td>
...
<?php
  ...
  case 3:  // Editar
    $nnn = 'edita';
    $strAction = "Editar";
    $tit   = "Edici&oacute;n tipo documento";
  ...
  case 4: // Nuevo registro, revisar     
    $nnn = 'new';
    $strAction = "Nuevo";
    $tit   = "Adici&oacute;n tipo documento";
...
function fnread($data)
{
  try {
    ...
    $obdata = new clstipodocumento($data);
...
function generaHTML($xx, $tit, $strAction, $strb1, $strb2, $idbusca, $nnn, $onlyread)
{
  ...
  if ($xx != 2) { //cuando no es un nuevo registro    
    ...
    if ($n == 0) { // no se encuentra el registro
      $p = "tipodoc";
      $enlace = "?page=frmgraltipodocumento&op=1";
      include('../plantillas/paso.php');
    } else $rs = mysqli_fetch_assoc($obfn);    
  }
  $headform = '<form autocomplete="off" action="../Controllers/registrotipoDocumentoController.php" method="post">';
  $strHTML = "";
  $strHTML =
    '<div class="box box-default">
      <div class="box-header with-border">
              ...
              <div class="row">
                ...
                <div class="col-md-8">
                  <div class="form-group has-feedback">
                    <label>Descripci&oacute;n tipo documento</label>
                    <input type="text" name="Descripcion" id="Descripcion" class="form-control" 
                    placeholder="Descripcion" title = "Descripci&oacute;n tipo documento"
                    value="' . (($xx == 1) ? $rs['Descripcion'] : '') . '" required
                    ' . $onlyread . '>
